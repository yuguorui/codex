...
directly. Finish with `return value`; that value becomes the workflow result.
There is no filesystem, process, network, or other Node.js API. `import()` is unavailable.
...
failure returns `null`; a stalled agent is automatically retried up to three times with exponential
backoff, and if it still makes no progress the run pauses and waits for you to retry or skip that
agent. Settled agents (completed, skipped, or failed) stay re-executable: retrying one restarts the
run from that agent, replays earlier agents from the run journal, and recomputes downstream stages
that already ran, while stages that had not run yet simply run with the new value. Filter nullable
results before dereferencing them.
