...
impl WorkflowResultData {
    ...
    ///
    /// Why an inline read or write failed matters less than why the run failed, and
    /// `nextAction` already says what to try next, so the ladder spends this before
    /// it shortens the run error.
