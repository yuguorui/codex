...
pub(crate) const WAIT_MODEL_CONTEXT_COMPACT_BYTES: usize = 768;
...
impl WorkflowResultData {
    ...
    pub(super) fn from_snapshot_with_result(
        ...
        let Some(chunk) = chunk else {
            ...
            if result_error.is_some() {
                unavailable.next_action = Some("ReadWorkflowResult: offset=0.".to_string());
            }
    ...
    pub(super) fn from_write_error(snapshot: &WorkflowTaskSnapshot, error: &str) -> Self {
        Self {
            ...
            next_action: Some("Fix writePath and repeat WaitWorkflow.".to_string()),
        }
    }
    ...
    pub(super) fn compact_for_wait(&mut self) {
        ...
        if self.result_available {
            ...
            self.next_action = Some("ReadWorkflowResult: offset=0 or writePath.".to_string());
        }
        self.result_error = self
            .result_error
            .as_deref()
            .map(|error| truncate_model_text(error, /*max_bytes*/ 8));
    }
...
pub(super) fn workflow_result_is_writable(snapshot: &WorkflowTaskSnapshot) -> bool {
