...
impl WaitWorkflowOutput {
    ...
    pub(super) fn from_outcome_with_result(
        ...
        if serde_json::to_vec(&output)?.len() > WAIT_MODEL_CONTEXT_COMPACT_BYTES {
            ...
            // Compaction must never grow the item. Dropping a small inline result
            // while adding a longer `nextAction` would make the response both bigger
            // and less useful, and cost the model a read it did not need.
            if serde_json::to_vec(&output)?.len() >= serde_json::to_vec(&uncompacted)?.len() {
                output = uncompacted;
            }
