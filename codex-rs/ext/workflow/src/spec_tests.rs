...
#[test]
fn workflow_tool_text_guides_structured_inputs_and_owning_model_control() {
    ...
    assert!(spec.description.contains("Pass all runtime data"));
    assert!(spec.description.contains("named `inputs`"));
    assert!(spec.description.contains("owning model retains"));
    assert!(
        properties["args"]
            .description
            .as_deref()
