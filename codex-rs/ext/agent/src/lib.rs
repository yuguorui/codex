...
impl AgentRunner {
    ...
    pub async fn run_to_completion_with_options(
        ...
        let run = self
            ...
            .await?;
        self.wait_for_completion(run, progress_timeout, cancellation)
            .await
    }
    ...
    async fn wait_for_completion(
        ...
        loop {
            ...
            let event = tokio::select! {
                event = run.thread.next_event() => event?,
            ...
            AgentSpawnMode::FreshSubagent {
                ...
                if source.trim().is_empty() {
                    return Err(CodexErr::InvalidRequest(
                        "fresh subagent source must not be empty".to_string(),
                    ));
                }
                start_options.session_source =
                    Some(SessionSource::SubAgent(SubAgentSource::Other(source)));
