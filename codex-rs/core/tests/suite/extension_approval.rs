...
const GUARDIAN_ARTIFACT_PAGE_BYTES: usize = 8 * 1_024;
...
async fn run_probe(review: ReviewCase) -> Result<ProbeResult> {
    ...
    if !matches!(&review, ReviewCase::AllowWithoutReadingArtifact) {
        ...
        while offset < artifact_contents.len() {
            ...
            offset = offset
                .saturating_add(GUARDIAN_ARTIFACT_PAGE_BYTES)
                .min(artifact_contents.len());
...
async fn extension_workflow_approval_routes_automatic_approve_through_guardian() -> Result<()> {
    ...
    assert!(guardian_request.body_contains_text("guardian_extension_approval"));
...
async fn large_extension_workflow_action_is_pageable_by_guardian() -> Result<()> {
    ...
    for offset in (0..serde_json::to_string(&approval_action(true))?.len())
        .step_by(GUARDIAN_ARTIFACT_PAGE_BYTES)
    {
        ...
        assert!(codex_utils_output_truncation::approx_token_count(&output) < 10_000);
