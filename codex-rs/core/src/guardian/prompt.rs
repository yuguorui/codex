...
pub(crate) async fn build_guardian_prompt_items_with_parent_turn(
    ...
    let transcript_entries = collect_guardian_transcript_entries_excluding_call(
...
pub(crate) fn collect_guardian_transcript_entries(
    ...
    collect_guardian_transcript_entries_excluding_call(history, node_repl_result_token_limit, None)
}

/// [`collect_guardian_transcript_entries`] variant that also drops the tool
/// call and its matching result for one excluded call ID. Hash-bound extension
/// approvals exclude the pending call because the action is presented separately
/// via `read_guardian_approval_artifact`.
pub(crate) fn collect_guardian_transcript_entries_excluding_call(
    history: &dyn SectionHistory,
    node_repl_result_token_limit: usize,
    ...
    let history = FilteredGuardianHistory {
        history,
        excluded_call_id,
    };
