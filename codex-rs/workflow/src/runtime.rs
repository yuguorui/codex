...
///
/// Implementations may invoke this periodically (ignoring snapshots that carry no new
/// usage) so hosts can refresh progress displays without waiting for the attempt to finish.
pub type WorkflowAgentProgressCallback<'a> = Box<dyn Fn(WorkflowTokenUsage) + Send + Sync + 'a>;
...
pub struct WorkflowRuntimeConfig {
    ...
    /// Cap for user-requested agent retries via the retry/skip controls.
...
impl ControlState {
    ...
    fn set_rerun_from(&self, index: usize) -> bool {
        *self
        ...
        self.rerun_notify.notify_one();
        true
    }

    fn take_rerun_from(&self) -> Option<usize> {
        self.rerun_from
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .take()
    ...
    fn rerun_notified(&self) -> impl Future<Output = ()> + Send + '_ {
        self.rerun_notify.notified()
    }
}
...
pub async fn execute_workflow(
    ...
    loop {
        let rerun_from = control.state.take_rerun_from();
        delegate.begin_session(rerun_from);
        ...
        .await;
        match result {
            Ok(_) if control.state.rerun_pending() => {
                delegate.record_log(
...
async fn run_workflow_source(
    ...
    let mut response = tokio::select! {
        ...
        _ = control.rerun_notified(), if observe_rerun => {
            let _ = session.terminate(cell_id.clone()).await;
            let _ = session.shutdown().await;
...
impl WorkflowDelegate {
    fn new(
        ...
        Self {
            ...
            invocation_state: Mutex::new(InvocationState {
                agent_count: 0,
                previous_cache_key: cache_root.clone(),
                rerun_from: None,
            }),
            cache_root,
    ...
    /// Resets per-session invocation state. A `rerun_from` index re-executes that
    /// agent and everything after it while replaying earlier agents from the journal.
    fn begin_session(&self, rerun_from: Option<usize>) {
        *self
            .invocation_state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = InvocationState {
            agent_count: 0,
            previous_cache_key: self.cache_root.clone(),
            rerun_from,
        };
        self.child_session_count.store(0, Ordering::Relaxed);
    }
