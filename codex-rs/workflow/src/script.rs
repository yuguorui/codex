...
use crate::scope_analysis::find_unavailable_global;
...
pub enum WorkflowScriptError {
    #[error("workflow script exceeds the 512 KiB limit")]
    TooLarge,
    #[error("workflow script contains a disallowed control character at byte {0}")]
    ...
    InvalidMeta(String),
    #[error("workflow metadata field `{0}` must not be empty")]
    EmptyMetaField(&'static str),
    #[error("workflow metadata field `{field}` exceeds the {max_bytes}-byte limit")]
    MetaFieldTooLarge {
        field: &'static str,
        max_bytes: usize,
    },
    #[error("workflow metadata contains too many phases (maximum {MAX_PHASES})")]
    TooManyPhases,
    #[error("workflow script uses nondeterministic API `{0}`")]
    ...
    #[error(
        "workflow script uses unavailable outer API `{name}` at line {line}, column {column}. {guidance}"
    )]
    UnavailableGlobal {
        name: String,
        line: usize,
...
pub fn validate_workflow_script(
    ...
    if let Some(unavailable) = find_unavailable_global(body) {
    ...
    Ok(ValidatedWorkflowScript { source, body, meta })
}
...
pub(crate) struct WorkflowScriptContext {
    ...
    pub(crate) initial_spent_tokens: u64,
}
...
pub(crate) fn compile_workflow_source_with_context(
    ...
    let args = serde_json::to_string(args)
        .map_err(|error| WorkflowScriptError::InvalidMeta(error.to_string()))?;
    let token_budget = token_budget.map_or_else(|| "null".to_string(), |value| value.to_string());
...
const {{ agent, parallel, pipeline, phase, log, budget, workflow }} = __wfBuildApi();
