...
pub(super) fn release_is_complete(release_dir: &Path, version: &str, target: &str) -> Result<bool> {
    if !release_dir.join("codex-package.json").is_file() {
        return Ok(false);
    }
    Ok(validate_release(release_dir, version, target).is_ok())
}

pub(super) fn validate_release(release_dir: &Path, version: &str, target: &str) -> Result<()> {
    ...
    if manifest.version != version || manifest.target != target {
        bail!(
            "package metadata does not match release {version} for {target}: found {} for {}",
            manifest.version,
            manifest.target
        );
    }
