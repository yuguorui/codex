...
`WaitWorkflows` reports status for every requested run in request order, and never returns result
content for `mode: "all"`. With `mode: "any"` it also returns `winner`: the run that ended the wait,
carrying the same bounded result head `WaitWorkflow` would have returned, so a race does not cost a
second round trip. `winner` is `null` for `mode: "all"`, when no run satisfied the condition, and
when it had to be dropped to fit the response size cap; any `workflows` entry with `timedOut: false`
is then a valid run to read next.
