...
pub(super) fn workflow_agent_detail_params(
    ...
    if task.status.is_active() && agent.state == WorkflowAgentState::Start {
...
fn agent_subtitle(agent: &WorkflowAgentProgress) -> String {
    ...
    match (agent.phase_index, agent.phase_title.as_deref()) {
        (Some(index), Some(title)) => format!("{status} · Phase {}: {title}", index + 1),
        (Some(index), None) => format!("{status} · Phase {}", index + 1),
        (None, Some(title)) => format!("{status} · {title}"),
