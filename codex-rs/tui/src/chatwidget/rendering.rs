...
impl ChatWidget {
    pub(crate) fn as_renderable(&self) -> RenderableItem<'_> {
        let active_cell_right_reserve =
            self.ambient_pet_wrap_reserved_cols(self.last_rendered_width.get().unwrap_or(u16::MAX));
...
impl Renderable for TextPetRenderable<'_> {
    ...
    fn desired_height(&self, width: u16) -> u16 {
        ...
        if !self.visible || self.pet.hidden_at_width(width) || width < pet_columns {
            return child_height;
        }
