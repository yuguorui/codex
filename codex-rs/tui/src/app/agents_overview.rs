...
fn agents_daemon_executable(current_executable: PathBuf) -> PathBuf {
    let is_codex_tui = current_executable
        .file_stem()
    ...
    let codex_plus = current_executable.with_file_name(if cfg!(windows) {
        "codex++.exe"
    } else {
        "codex++"
    });
    if codex_plus.is_file() {
        ...
        current_executable.with_file_name(if cfg!(windows) { "codex.exe" } else { "codex" })
    }
}
