...
const handleEvent = (event: ThreadEvent): void => {
  switch (event.type) {
    ...
    case "turn.completed":
      console.log(
        `Used ${event.usage.input_tokens} input tokens, ${event.usage.cached_input_tokens} cached input tokens, ${event.usage.cache_write_input_tokens} cache write input tokens, ${event.usage.cache_creation_input_tokens} cache creation input tokens, ${event.usage.output_tokens} output tokens, ${event.usage.reasoning_output_tokens} reasoning output tokens.`,
      );
      break;
    case "turn.failed":
